...
This repository contains the code and data accompanying our paper on using **Localized Cross-Correlations (LCC)** to measure glacier ice flow velocity from repeated Ground Penetrating Radar (GPR) profiles.

## Overview

Traditional image matching techniques apply a single, global correlation between two images. Our approach computes cross-correlations that are spatially *localized* using Gaussian windows, allowing the method to capture spatially varying displacements — well-suited to the heterogeneous deformation patterns found in glacier ice.

The method is applied to repeat GPR surveys of the Findelen glacier (Switzerland), where the subsurface ice structure recorded in two surveys separated in time is used to recover a 2D displacement field. We validate the approach on synthetic GPR data with a known ground-truth velocity field before applying it to the real field data.
...
├── optimizer.py          # Variational refinement of the displacement field (ShiftOptim)
...
A CUDA-capable GPU is required to run the LCC processing scripts.
